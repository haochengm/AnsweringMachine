...
namespace Meadow.Studio
{
    ...
    {   
        ...
        private static void OnEditorUpdate()
        {
            ...
            if(currentPage == SetupPage.Loading && setupMetadata != null && setupMetadata.SetupComplete && !setupMetadata.RefreshAssetDatabase && !setupProcessActive && !setupMetadata.InitializeVisualScripting && !setupMetadata.UpdateVisualScriptingForInstalledPackages && !setupMetadata.RegenerateVisualScriptingNodes)
            {
                ...
                window.CreateSetupCompleteUI();
             
                setupMetadata.UpdatingConfig = false;
                WriteMeadowSetupData(setupMetadata);
            }
            
            //Handle package specific scripting defines
            HandlePackageDefines();

            //Check for installed optional packages
            CheckForInstalledOptionalPackages();
        }
        private static void HandleSetupFlags()
        {
            ...
            {   
                ...
                if(setupMetadata.UpdateVisualScriptingForInstalledPackages)
                {
                    ...
                    // Update the visual scripting settings for the core package
                    if(setupConfigJson.core.visualScripting != null)
                    {
                        UpdateVisualScriptingSettings(setupConfigJson.core.visualScripting);
                    }
        ...
        private static void HandlePackageDefines()
        {
            if(setupConfigJson != null && setupMetadata != null)
            {
                ...
                if(setupMetadata.SetupComplete)
                {
                    ...
                    if(setupConfigJson.core.packageDefineHandlers != null)
                    {
                        foreach(string define in setupConfigJson.core.packageDefineHandlers.Keys)
                        {
                            //if the scripting define does not exist add it
                            bool requirementsMet = CheckRequirements(setupConfigJson.core.packageDefineHandlers[define].requirements, jsonText, setupConfigJson.core.packageDefineHandlers[define].anyRequirement);
        ...
        {
            // Debug.Log("Setting scripting define symbol: " + define + " to " + addDefine + " for target: " + target);
        ...
        [MenuItem("Meadow/Optional Packages", false, 101)]
        private static void OpenOptionalPackages()
        {
            //check if the editor is in playmode
            if (EditorApplication.isPlaying)
            {
                //create a prompt warning the user that they need to exit playmode and ask if they want to exit playmode 
                if (EditorUtility.DisplayDialog("Meadow Warning", "You need to stop playmode to install optional packages", "Stop Playmode", "Cancel"))
                {
            ...
            MeadowSetupWindow wnd = CreateWindow("Meadow Setup", 800, 613);
            wnd.titleContent = new GUIContent("Optional Packages");
            wnd.CreateOptionalPackagesUI();
        }

        // [MenuItem("Meadow/Check Installed Optional Packages", false, 102)]
        private static void CheckForInstalledOptionalPackages()
        {
            if(setupConfigJson != null && setupMetadata != null && setupMetadata.SetupComplete)
            {
                //check if any of the optional packages are already installed before setup
                List<string> optionalPackagesInstalled = new List<string>();
                foreach (var package in setupConfigJson.optionalPackages.Keys)
                {
                    string packageName = setupConfigJson.optionalPackages[package].packages.dependencies.Keys.FirstOrDefault();
                    if(AssetDatabase.IsValidFolder("Packages/" + packageName) && !setupMetadata.OptionalPackagesInstalled.Contains(package))
                    {
                        optionalPackagesInstalled.Add(package);
                    }
                }
                
                if(optionalPackagesInstalled.Count > 0)
                {
                    InstallOptionalPackages(optionalPackagesInstalled);
                }
            }
        }

        private static MeadowSetupWindow CreateWindow(string title, float width = 800, float height = 400)
        {
        ...
        private static void ClearSetupData()
        {
            ...
            string projectDirectory = Path.Combine(Application.persistentDataPath, projectName);
            if(!Directory.Exists(projectDirectory))
            {
                Directory.CreateDirectory(projectDirectory);
            }
            string path = Path.Combine(projectDirectory, "meadow-setup.data");
        ...
        // [MenuItem("Meadow/debug complete window", false, 104)]
        private static void OpenCompleteUIDebug()
        {
            MeadowSetupWindow wnd = CreateWindow("Meadow Setup");
            wnd.titleContent = new GUIContent("Meadow Setup");
            wnd.minSize = new Vector2(400, 200);
            wnd.CreateSetupCompleteUI();
        }
    ...
    #region UI
        private void CreateOptionalPackagesUI()
        {
            if(setupMetadata == null)
            {
                setupMetadata = LoadMeadowSetupData();
            ...
            //!HOLD getting the package metadata from the upm server - currently blocked by SSL certificate issues with HTTP requests
            // string package = setupConfigJson.optionalPackages.Keys.First();
            // UnityWebRequest request = UnityWebRequest.Get("http://35.189.115.164:4873/com.untoldgarden.xref-experience-builder");
            // request.SendWebRequest().completed += (AsyncOperation op) => 
            // {
            //     if(request.result == UnityWebRequest.Result.ConnectionError || request.result == UnityWebRequest.Result.ProtocolError)
            //     {
            //         // Debug.LogError("Error loading package metadata: " + request.error);
            //         return;
            //     }
            //     else
            //     {
            ...
            //         Debug.Log("Latest version: " + jsonObject["dist-tags"]["latest"].ToString());
            //     }
            // };

            //Create an optional package card in the packages-container for each optional package
            foreach (var package in setupConfigJson.optionalPackages.Keys)
            {
                //create the package card
                VisualElement packageContainer = rootVisualElement.Q<VisualElement>("package-container");
                VisualElement packageCardContainer = new VisualElement();
                packageContainer.Add(packageCardContainer);
                packageCardContainer.name = package;
                VisualTreeAsset packageCard = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>("Assets/Meadow-Studio/UI/Wizard/package-card.uxml");
                packageCard.CloneTree(packageCardContainer);

                //set the package name and description
                packageCardContainer.Q<Label>("package-name").text = setupConfigJson.optionalPackages[package].packageMetadata.name;
                packageCardContainer.Q<Label>("package-description").text = setupConfigJson.optionalPackages[package].packageMetadata.description;

                //handle pro label
                if(setupConfigJson.optionalPackages[package].packageMetadata.isPro)
                {
                    packageCardContainer.Q<VisualElement>("pro-label-container").style.display = DisplayStyle.Flex;
                }
                else
                {
                    packageCardContainer.Q<VisualElement>("pro-label-container").style.display = DisplayStyle.None;
                }

                //handle more info button
                Label moreInfoButton = packageCardContainer.Query<Label>("package-more-info");
                if(setupConfigJson.optionalPackages[package].packageMetadata.moreInfoUrl != null && setupConfigJson.optionalPackages[package].packageMetadata.moreInfoUrl != "")
                {
                    moreInfoButton.style.display = DisplayStyle.Flex;
                    moreInfoButton.RegisterCallback<MouseUpEvent>(evt => 
                    {
                        Application.OpenURL(setupConfigJson.optionalPackages[package].packageMetadata.moreInfoUrl);
                    });
                }
                else
                {
                    moreInfoButton.style.display = DisplayStyle.None;
                }

                //handle coming soon label
                if (setupConfigJson.optionalPackages[package].packageMetadata.comingSoon)
                {
                    packageCardContainer.Q<VisualElement>("package-coming-soon-label").style.display = DisplayStyle.Flex;
                    //if the coming soon label is clicked open the support@meadow.space email
                    packageCardContainer.Q<Label>("package-coming-soon-label").RegisterCallback<MouseUpEvent>(evt =>
                    {
                        // Debug.Log("Coming soon label clicked");
                        string packageName = setupConfigJson.optionalPackages[package].packageMetadata.name;
                        packageName = packageName.Replace(" ", "%20");
                        Application.OpenURL("mailto:support@meadow.space?subject=Package%20Request%20-%20" + packageName + "&body=I%20would%20like%20to%20request%20the%20" + packageName + "%20package%20for%20Meadow.");
                    });
                }
                else
                {
                    packageCardContainer.Q<VisualElement>("package-coming-soon-label").style.display = DisplayStyle.None;
                }

                //handle the package image
                if (setupConfigJson.optionalPackages[package].packageMetadata.imageUrl != null && setupConfigJson.optionalPackages[package].packageMetadata.imageUrl != "")
                {
                    UnityWebRequest request = UnityWebRequest.Get(setupConfigJson.optionalPackages[package].packageMetadata.imageUrl);
                    request.SendWebRequest().completed += (AsyncOperation op) =>
                    {
                        if (request.result == UnityWebRequest.Result.ConnectionError || request.result == UnityWebRequest.Result.ProtocolError)
                        {
                            // Debug.LogError("Error loading image: " + request.error);
                            //use the optional-package-default-image
                            Texture2D defaultImage = AssetDatabase.LoadAssetAtPath<Texture2D>("Assets/Meadow-Studio/Resources/optional-package-default-image.png");
                            packageCardContainer.Q<VisualElement>("package-image").style.backgroundImage = defaultImage;
                            packageCardContainer.Q<VisualElement>("package-image").style.display = DisplayStyle.Flex;
                        }
                        else
                        {
                            ...
                            packageCardContainer.Q<VisualElement>("package-image").style.backgroundImage = texture;
                            packageCardContainer.Q<VisualElement>("package-image").style.display = DisplayStyle.Flex;
                        }
                    };
                }
                ...
                {
                    packageCardContainer.Q<VisualElement>("package-image").style.display = DisplayStyle.None;
                }

                //set up the toggle button
                Toggle packageToggle = packageCardContainer.Query<Toggle>("package-toggle");

                //check if the package is already installed
                if(setupMetadata.OptionalPackagesInstalled.Contains(package))
                {
                    packageToggle.value = true;
                    packageToggle.SetEnabled(false);
                    packageToggle.style.opacity = 0.5f;
                }
                else
                {
                    packageToggle.value = false;
                    packageToggle.SetEnabled(true);
                    packageToggle.style.opacity = 1f;

                    packageToggle.RegisterValueChangedCallback((evt) => 
                    {
                        //Add logic to include this optional package in the setup process
                        if(evt.newValue && !optionalPackagesSelected.Contains(package))
                            optionalPackagesSelected.Add(package);
                        else if(!evt.newValue && optionalPackagesSelected.Contains(package))
                            optionalPackagesSelected.Remove(package);
                    });
                }
                ...
                {
                    packageToggle.SetEnabled(false);
                    packageToggle.style.opacity = 0.25f;
                    //set package image to 0.25 opacity
                    packageCardContainer.Q<VisualElement>("package-image").style.opacity = 0.5f;
                }
            }

            //set up the install button
            Button installButton = rootVisualElement.Query<Button>("install-button");
            installButton.clickable.clicked += InstallButtonClicked;
        }
        private async void InstallButtonClicked()
        {
            ...
            if(EditorUtility.DisplayDialog("Meadow Setup Warning", "Project settings will be changed. \nPlease ensure that you have a backup.", "Proceed", "Cancel"))
            {
                ...
                InstallOptionalPackages(optionalPackagesSelected);
        ...
        private void CreateSetupCompleteUI()
        {
            ...
            Label optionalPackagesTitle = optionPackagesContainer.Query<Label>("optional-packages-title");
            var installedPackagesDistinct = setupMetadata.OptionalPackagesInstalled.Distinct();
            optionalPackagesTitle.text = "Optional Packages: " + installedPackagesDistinct.Count();
            Label optionalPackagesBody = optionPackagesContainer.Query<Label>("added-packages-body");
            optionalPackagesBody.text = "";
            ...
            corePackagesBody.text += "\n-com.untoldgarden.meadow : " + setupConfigJson.core.packages.dependencies["com.untoldgarden.meadow"];
            ...
            if (setupMetadata != null && setupConfigJson != null && setupMetadata.OptionalPackagesInstalled != null && setupMetadata.OptionalPackagesInstalled.Count > 0)
            {
                ...
                foreach(var package in setupMetadata.OptionalPackagesInstalled.Distinct())
                {
                    if(setupConfigJson.optionalPackages.ContainsKey(package))
                    {
                        string packageName = setupConfigJson.optionalPackages[package].packages.dependencies.Keys.FirstOrDefault();
                        if (firstPackage)
                        {
                            optionalPackagesBody.text = packageName + " : " + setupConfigJson.optionalPackages[package].packages.dependencies[packageName];
                            firstPackage = true;
                        }
                        else
                        {
                            optionalPackagesBody.text += "\n-" + packageName + " : " + setupConfigJson.optionalPackages[package].packages.dependencies[packageName];
                        }
                    }
                }
        ...
        private static void InstallOptionalPackages(List<string> optionalPackages)
        {
            foreach (var package in optionalPackages)
            {
                if (setupConfigJson.optionalPackages.ContainsKey(package))
                {
                    ...
                    if (setupConfigJson.optionalPackages[package].visualScripting.assemblyOptions.Count > 0 || setupConfigJson.optionalPackages[package].visualScripting.typeOptions.Count > 0)
                        UpdateVisualScriptingSettings(setupConfigJson.optionalPackages[package].visualScripting);
            ...
            setupMetadata.OptionalPackagesInstalled.AddRange(optionalPackages);
            //remove duplicates from the optional packages list
            setupMetadata.OptionalPackagesInstalled = setupMetadata.OptionalPackagesInstalled.Distinct().ToList();

            //regenerate the visual scripting nodes
            setupMetadata.RegenerateVisualScriptingNodes = true;

            //refresh the asset database
            setupMetadata.RefreshAssetDatabase = true;

            //write the setup metadata
            WriteMeadowSetupData(setupMetadata);
        }
        public static void CoreSetupProcess()
        {
            setupProcessActive = true;
            ...
            if(setupConfigJson.core.visualScripting.assemblyOptions.Count > 0 || setupConfigJson.core.visualScripting.typeOptions.Count > 0)
                UpdateVisualScriptingSettings(setupConfigJson.core.visualScripting);
        ...
        public static void ConfigUpdateProcess()
        {
            setupProcessActive = true;
            ...
            if(setupMetadata.SetupComplete)
            {
                ...
                {
                    InstallOptionalPackages(setupMetadata.OptionalPackagesInstalled);
                }
        ...
        {
            #if UNITY_VISUAL_SCRIPTING
            if(setupMetadata != null && VSUsageUtility.isVisualScriptingUsed)
            {
                ...
                if(!File.Exists(visualScriptingSettingsPath))
                {
                    // Debug.LogError("Visual Scripting settings file not found.");
                    return;
                }
    ...
    #region Setup Helpers
        ...
        {
            //if MEADOW_STUDIO TESTING use the default config file in the resources folder
            #if MEADOW_STUDIO_TESTING
            Debug.Log("Meadow studio testing enabled: Using the default config file in the resources folder");
            return File.ReadAllText("Assets/Meadow-Studio/Resources/meadow-unity-config.json");
            
            #else
            using UnityWebRequest unityWebRequest = UnityWebRequest.Get("https://firebasestorage.googleapis.com/v0/b/xref-client.appspot.com/o/plugins%2Fmeadow-studio%2Fconfig%2Fv1%2Fmeadow-unity-config.json?alt=media");
